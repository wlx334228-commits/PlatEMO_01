...
% Upper : 1 x D
    ...
    Upper = Upper(:)';
    Span = Upper - Lower;
    fixedMask = Span <= 1e-12;
    safeSpan = Span;
    safeSpan(fixedMask) = 1;

    % Shuffle parents before pairing to avoid deterministic adjacent mating.
    Parent = Parent(randperm(N),:);

    %% If N is odd, add one extra parent temporarily
    if mod(N,2) == 1
    ...
    % Interleave paired offspring so an odd-parent truncation does not
    % always discard the same offspring position.
    ...
    OffDec(:,fixedMask) = LowerM(:,fixedMask);

    Site = rand(originalN,D) < proM/D;
    Site(:,fixedMask) = false;
    mu   = rand(originalN,D);

    temp = Site & mu <= 0.5;
    OffDec(temp) = OffDec(temp) + ...
        SpanM(temp).* ...
